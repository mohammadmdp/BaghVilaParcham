...
/**
 ...
 <p>
 OkHttp buffers to the network interface but the network interface's default buffer size is
 sometimes
 set very high e.g. 512Kb which makes tracking upload progress impossible as the upload content is
 sitting in the network interface buffer waiting to be transmitted. Re:
 https://github.com/square/okhttp/issues/1078
 <p>
 So here, we create socket factory that forces all sockets to have a restricted send buffer size. So
 that further down the chain in OkHttps' RequestBody we can track the actual progress to the nearest
 [sendBufferSize] unit.
 <p>
